...
from hrm.serializers import EmployeeTypeSerializer, ProductSerializer,CompanyTypeSerializer
from hrm.models import EmployeeType, Product,CompanyType
...
def company_table(request):
    url_path = request.build_absolute_uri(reverse('cmp_record'))
    response = requests.get(url_path, params=request.GET, json={})
    tabledetails = response.json()
    return render(request,'hrm/company_table.html',{'tabledetails': tabledetails})


def company_form(request, pk=''):
    cmp_type = CompanyType.objects.values_list('name', flat=True)
    product_list = Product.objects.values_list('name', flat=True)
    return render(request, 'hrm/company_form.html', {'pk': pk, 'cmp_type': cmp_type,'product_list':product_list})
